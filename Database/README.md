...
| File | Vai trò |
| --- | --- |
| schema.dbml | File nguồn dbdiagram.io |
| database.sql | DDL MariaDB dùng để deploy production |
...
- Xác thực qua email OTP (đăng nhập, đặt lại mật khẩu, xác minh email)

### 3.2. Các quan hệ chính

Schema gồm 38 bảng với các nhóm khóa ngoại chính:
...
```
EMAIL_OTP_CODES.user_id => USERS.id (nullable)
```

Mã OTP gửi qua email cho các mục đích đăng nhập, đặt lại mật khẩu, xác minh email. Hệ thống chỉ hỗ trợ kênh email, không hỗ trợ SMS. user_id có thể NULL nếu email chưa gắn tài khoản.
...
| email | VARCHAR(150) UNIQUE | Email đăng nhập và nhận OTP. Kênh xác thực chính, kể cả user vãng lai |
| password_hash | VARCHAR(255) NULL | Hash mật khẩu. User vãng lai luôn có mật khẩu tạm, nhận qua email để dùng cho lần đăng nhập tiếp theo |
...
- Xác thực OTP chỉ qua email (xem EMAIL_OTP_CODES), không hỗ trợ SMS
...
### 4.17. EMAIL_OTP_CODES

**Mục đích:** Mã OTP xác thực gửi qua email.

Hệ thống chỉ hỗ trợ OTP qua email, không hỗ trợ SMS. Dùng cho đăng nhập, đặt lại mật khẩu, xác minh email khi tạo tài khoản hoặc đổi email. Lưu hash của mã (không lưu plaintext), có expires_at và attempt_count để chặn brute-force.

| Cột | Kiểu | Mô tả |
| --- | --- | --- |
| id | INT PK | Khóa chính |
| email | VARCHAR(150) | Email nhận OTP. Không cần đã có tài khoản |
| user_id | INT FK => USERS NULL | NULL nếu email chưa gắn tài khoản |
| code_hash | VARCHAR(255) | Hash của mã OTP. Không lưu plaintext |
| purpose | ENUM(login, password_reset, email_verify) | Mục đích dùng OTP |
| expires_at | TIMESTAMP | Hết hạn (thường 5-10 phút) |
| consumed_at | TIMESTAMP NULL | Thời điểm dùng. NULL = chưa dùng |
| attempt_count | INT | Số lần nhập sai. Vượt ngưỡng thì khóa mã |
| created_at | TIMESTAMP | Thời điểm phát hành |

**Chỉ mục:**

- idx_otp_email_purpose (email, purpose)
...
- Mã chỉ dùng được 1 lần: khi verify thành công, set consumed_at = NOW()
- Từ chối nếu NOW() > expires_at hoặc consumed_at IS NOT NULL hoặc attempt_count >= 5
- Không xóa mã hết hạn ngay: cronjob dọn định kỳ để giữ audit
- User vãng lai: lần vào đầu tiên là auto-login khi quét QR (không qua OTP). Lần đăng nhập kế tiếp dùng mật khẩu tạm đã gửi qua email, hệ thống buộc đổi mật khẩu ngay (must_change_password = 1)
...
| login_method | ENUM(password, otp, walk_in_auto_login) | Phương thức login |
...
| metadata_json | JSON NULL | Metadata thêm: otp_purpose, event_id... |
...
2. Nếu chưa có => Tạo USERS (email lưu vào USERS.email, is_walk_in = 1, sinh mật khẩu tạm, password_hash = hash(temp), must_change_password = 1)
...
7. Lần đăng nhập lại sau bằng mật khẩu (login_method = password) hoặc email OTP (login_method = otp)
8. Email đã gắn vào tài khoản trở thành kênh nhận OTP cho các lần xác thực sau
...
1. User đăng nhập bằng password/OTP/walk-in auto
...
- Mọi thay đổi schema phải đồng bộ trên cả schema.dbml, database.sql và README này
...
1. **Xác thực user vãng lai:** Dùng mật khẩu tạm gửi qua email. Khi tạo tài khoản vãng lai, hệ thống sinh mật khẩu tạm và đặt USERS.must_change_password = 1, buộc đổi ở lần đăng nhập kế tiếp. Xác thực OTP chỉ qua email (bảng EMAIL_OTP_CODES), không hỗ trợ SMS
...
6. **Kênh OTP:** Chỉ hỗ trợ OTP qua email, không hỗ trợ OTP qua số điện thoại (SMS)
...
- **File nguồn:** [schema.dbml](schema.dbml) | [database.sql](database.sql)
